...
```
The "Tree" table corresponds to FAO data on forest cover for each country for the years 2015 to 2020, and the forest table corresponds to the World bank forest cover for 1991. Tree data sets is expressed in square kilometers (km2) and forest cover in ha.

(1)Our objective is to obtain the percentage of forest cover in comparison to cover forest of 1991. 
...
normall<-function(name)
{
  ...
  if (!is.na(Pays$Indicator[n]) & Pays$Indicator[n]=="state of forest reccources"){
   for (i in 3:8){
      ...
      table[n,i]<-ifelse(x>100,100,normhab(x,0,100))
    }
    n<-n+1
  }
